...
use gbrg_analyze::{
    analyze_file, analyze_file_report, analyze_path_report, build_whatif_graph,
    edges_with_cell_ids, AnalyzeOptions, AnalyzePathReport, DEFAULT_CHURN_WINDOW_DAYS,
};
        ...
        match analyze_path_report(&path, &config, DEFAULT_CHURN_WINDOW_DAYS) {
            Ok(report) => {
                ...
                if emit_edges {
                    ...
                    eprintln!(
                        "gbrg-analyze: --emit-edges → {} edge(s) surfaced",
                        edges.len()
                    );
