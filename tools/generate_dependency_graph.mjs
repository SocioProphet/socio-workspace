...
method: gitmodules+package+gomod-scan
basis: measured
scope:
...
summary:
  ...
  nodes: ${nodes.length}
by_type:
${['submodule','package','gomod'].map(t=>`  ${t}: ${edges.filter(e=>e.type===t).length}`).join('\n')}
first_party_by_type:
${['submodule','package','gomod'].map(t=>`  ${t}: ${internal.filter(e=>e.type===t).length}`).join('\n')}
