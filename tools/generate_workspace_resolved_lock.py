...
def require_names_resolved(lock: dict[str, Any], required_names: set[str]) -> tuple[bool, list[str]]:
    by_name = {repo["name"]: repo for repo in lock["repos"]}
    errors: list[str] = []
    for name in sorted(required_names):
        repo = by_name.get(name)
        if repo is None:
            errors.append(f"{name}: missing from lock")
            continue
        if repo["resolution_status"] != "resolved":
            errors.append(f"{name}: {repo['resolution_status']}: {repo.get('resolution_error')}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-names", nargs="*", default=[], help="specific repo names that must resolve")
    ...
    if args.require_names:
        ok, errors = require_names_resolved(lock, set(args.require_names))
        if not ok:
            for error in errors:
