...
def main() -> int:
    ...
    try:
        ...
        require(data.get("kind") == "ComputationalArtifactRegistry", "kind must be ComputationalArtifactRegistry")
        ...
        for cls in BLOCKED_AUTO_PROMOTE_CLASSES:
            ...
            require(isinstance(cls_def, dict), f"spec.safetyClasses.{cls} must be a mapping")
            require(
        ...
        require(isinstance(slash_binding.get("namespace"), str), "spec.governance.slashTopicBinding.namespace must be a string")
        require_list(slash_binding, "topics", "spec.governance.slashTopicBinding")
        require(isinstance(slash_binding.get("governingRepo"), str), "spec.governance.slashTopicBinding.governingRepo must be a string")
        ...
        for entry in entries:
            ...
            require(safety_cls in actual_safety, f"registryEntry '{entry_id}' has unknown safetyClass: {safety_cls}")
            consumers = entry.get("downstreamConsumers")
            require(isinstance(consumers, list) and bool(consumers), f"registryEntry '{entry_id}' downstreamConsumers must be non-empty list")
